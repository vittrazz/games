...
def play():
    ...
    word_completion = [new_word[0]] + list('_' * (len(word)-2)) + [new_word[-1]]  # строка, содержащая символы _ на каждую букву задуманного слова
    ...
    while guessed == False and tries > 0:
        ...
        if len(letter) > 1:
            ...
            if letter in new_word[1: -1]:
                print('Правильная буква!')
                while letter in new_word:
                    i = new_word.index(letter)
                    word_completion[i] = letter
                    new_word[i] = '*'
                print(*word_completion, sep=' ')
                print(f'количество попыток: {tries}')
            elif letter in guessed_letters[:-1] and letter not in new_word:
        ...
        if ''.join(word_completion) == word:
            ...
            break
    if guessed == True:
        print('Вы угадали!!! Поздравляем!!! Умничка наша!')
        print(answers())    
    elif guessed == False and tries == 0:
        print('Вы проиграли!', f'Загаданное слово: {word}', sep='\n')
        print(answers())
